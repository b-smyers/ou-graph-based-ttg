...
if not CATALOG_PATH:
    ...
    exit()
if not URI:
    logger.error("Missing NEO4J_DB_URI. Did you set the env?")
    exit()
if not AUTH[0] or not AUTH[1]:
    logger.error("Missing NEO4J_USERNAME or NEO4J_PASSWORD. Did you set the env?")
    exit()
...
def create_level(tx, level):
    ...
    if level not in level_to_credits:
        logger.error(
            f"'{level}' did not match any level options: {', '.join(list(level_to_credits.keys()))}"
        )
        return None
...
def process_course_requisites(tx, course):
    ...
    if node is None:
        logger.error("Could not find previously imported course.")
...
def process_requisite(tx, parent_req, parent_uuid):
    ...
    if t == "PERMISSION":
        ...
        if permission_authority is not None:
            ...
            logger.error("'authority' property was expected, but was not found")
        ...
        if gpa is not None:
            ...
            logger.error("'gpa' property was expected, but was not found")
        ...
        if course_code is not None:
            ...
            if node is None:
                logger.warn(
                    f"Could not find requisite course with course code '{course_code}'."
                )
                return
            ...
            logger.error("'course' property was expected, but was not found")

    elif t == "PLACEMENT":
        placement_subject = parent_req.get("subject", None)
        placement_level = parent_req.get("level", None)
        if placement_subject is None:
            logger.error("'placement' property was expected, but was not found")
            return
        if placement_level is None:
            logger.error("'level' property was expected, but was not found")
            return
        ...
        if requirements is not None:
            ...
            logger.error("'requirements' property was expected, but was not found")
        ...
        if other is not None:
            ...
            logger.error("'other' property was expected, but was not found")
...
def main():
    with driver.session() as session:
        ...
        logger.info(f"Creating {total_courses} course nodes.")
        # First pass: Create all course nodes
        for i, course in enumerate(catalog):
            if i % 100 == 0:
                logger.info(
                    f"Creating courses {i}/{total_courses} - {round(100 * i / total_courses, 2)}%"
                )
            session.execute_write(create_course, course)

        logger.info("Creating requisites relationships.")
        # Second pass: Process requisites
        for i, course in enumerate(catalog):
            if i % 100 == 0:
                logger.info(
                    f"Processing course requisites {i}/{total_courses} - {round(100 * i / total_courses, 2)}%"
                )
            session.execute_write(process_course_requisites, course)

    logger.info("Catalog loaded into Neo4j.")
